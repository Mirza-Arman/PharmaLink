...
router.get('/requests', auth('pharmacy'), async (req, res) => {
  try {
    ...
    // Filter out requests that this pharmacy has already rejected
    const filtered = requests.filter(r => !Array.isArray(r.rejectedPharmacies) || !r.rejectedPharmacies.includes(req.user.id.toString()));
    
    // For each request, also fetch bills to show pharmacy-specific status
    const requestsWithBills = await Promise.all(filtered.map(async (request) => {
...
// Reject request (pharmacy declines to bid). Mark pharmacy as rejected and set any existing bills to status 'rejected'.
router.post('/reject-request/:requestId', auth('pharmacy'), async (req, res) => {
  try {
    ...
    // Check if pharmacy is in selectedPharmacies array
    if (!request.selectedPharmacies.includes(pharmacyId.toString())) {
      return res.status(403).json({ message: 'You are not authorized to reject this request' });
    }

    // Mark this pharmacy as rejected without removing from selectedPharmacies
    if (!Array.isArray(request.rejectedPharmacies)) {
      request.rejectedPharmacies = [];
    }
    if (!request.rejectedPharmacies.includes(pharmacyId.toString())) {
      request.rejectedPharmacies.push(pharmacyId.toString());
    }

    // Defensive: ensure this pharmacy stays listed in selectedPharmacies so the customer can see it with Rejected status
    if (!Array.isArray(request.selectedPharmacies)) {
      request.selectedPharmacies = [];
    }
    if (!request.selectedPharmacies.includes(pharmacyId.toString())) {
      request.selectedPharmacies.push(pharmacyId.toString());
    }

    // If every selected pharmacy has rejected, update overall request status
    const allRejected = request.selectedPharmacies.length > 0 && request.selectedPharmacies.every(id => request.rejectedPharmacies.includes(id.toString()));
    if (allRejected && request.status === 'pending') {
      request.status = 'rejected';
    }
    await request.save();

    // Mark any existing bills from this pharmacy as rejected (do not delete so the customer can see rejection)
    ...
    return res.json({ 
      message: 'You have declined to bid on this request',
      requestStatus: request.status,
      remainingPharmacies: request.selectedPharmacies.length
    });
  } catch (err) {
...
// Delete request - Remove pharmacy from selectedPharmacies array
router.delete('/delete-request/:requestId', auth('pharmacy'), async (req, res) => {
  try {
    const { requestId } = req.params;
    const pharmacyId = req.user.id;
    ...
    // Treat delete as a local dismissal: mark as rejected for this pharmacy so it disappears from their dashboard
    if (!Array.isArray(request.rejectedPharmacies)) {
      request.rejectedPharmacies = [];
    }
    if (!request.rejectedPharmacies.includes(pharmacyId.toString())) {
    ...
    // Do not remove from selectedPharmacies; ensure customer still sees the pharmacy with Rejected status
    await request.save();

    // Mark any existing bills from this pharmacy as rejected (don't delete)
    await Bill.updateMany({ 
      request: requestId, 
      pharmacy: pharmacyId
    }, { status: 'rejected' });

    res.json({ message: 'Request hidden from your dashboard. Customer will see status as Rejected.' });
  } catch (err) {
