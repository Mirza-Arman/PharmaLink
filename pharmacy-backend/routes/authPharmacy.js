...
// Reject request (pharmacy declines to bid). Remove pharmacy from selectedPharmacies.
router.post('/reject-request/:requestId', auth('pharmacy'), async (req, res) => {
  try {
    ...
    // Remove pharmacy from selectedPharmacies array
    request.selectedPharmacies = request.selectedPharmacies.filter(id => id.toString() !== pharmacyId.toString());
    
    // If no pharmacies left in selectedPharmacies, mark request as rejected
    if (request.selectedPharmacies.length === 0) {
      request.status = 'rejected';
    }
    
    await request.save();

    // Clean up any bills generated by this pharmacy for this request
...
router.delete('/delete-request/:requestId', auth('pharmacy'), async (req, res) => {
  try {
    ...
    // Check if request has accepted bills - prevent deletion if customer accepted this pharmacy's bill
    if (request.status === 'accepted' && request.acceptedBy && request.acceptedBy.toString() === pharmacyId.toString()) {
      return res.status(400).json({ message: 'Cannot delete request with accepted bill' });
    }
    ...
    res.json({ message: 'Request removed from your dashboard successfully' });
  } catch (err) {
